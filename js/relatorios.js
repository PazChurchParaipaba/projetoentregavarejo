...
const RelatoriosEnterprise = {
    ...
    exibirModalReposicao: (titulo, subtitulo, cabecalhoHtml, linhas, rodapeHtml, totalGeral) => {
        ...
        modal.innerHTML = `
            <div class="modal-content" style="background:#1a1a1a; color:#fff; width:98%; max-width:1200px; border:1px solid #333; border-radius:8px;">
    ...
    relatorioInventario: async () => {
        ...
        RelatoriosEnterprise.exibirModal(`📦 Estoque Atual`, new Date().toLocaleDateString('pt-BR'), linhas, null);
    },

    // 📈 RELATÓRIO DE ESTOQUE MENSAL E VARIAÇÕES
    relatorioEstoqueMensal: async () => {
        if (!await RelatoriosEnterprise.checkStore()) return;
        App.utils.toast("Analisando estoque e vendas do mês...", "info");

        // Busca produtos e vendas do último mês
        const hoje = new Date();
        const trintaDiasAtras = new Date(hoje.getTime() - (30 * 24 * 60 * 60 * 1000)).toISOString();

        const { data: produtos } = await _sb.from('products').select('*').eq('store_id', App.state.storeId);
        ...
        const resumo = produtos.map(p => {
            ...
            if (vendas) {
                vendas.forEach(v => {
                    try {
                        ...
                        const itens = obs.items || obs.itens || [];
                        itens.forEach(i => {
                            if (i.id === p.id || i.nome === p.nome) vendidos += (parseFloat(i.qtd) || 1);
                        });
                    } catch (e) {}
                });
            }
            return {
                ...
                atual: p.estoque || 0,
                vendido: vendidos,
                valor: (p.estoque || 0) * (p.preco || 0)
            };
        });

        const totalValorEstoque = resumo.reduce((acc, r) => acc + r.valor, 0);

        const linhas = resumo.sort((a,b) => b.vendido - a.vendido).map(r => `
            <tr>
                <td>${r.nome}</td>
                <td align="right">${r.atual}</td>
                <td align="right" style="color:#4ade80;">+${r.vendido}</td>
                <td align="right">R$ ${r.valor.toFixed(2)}</td>
            </tr>
        `).join('');
        ...
        const htmlTabela = `
            <div style="margin-bottom:20px; display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                <div style="background:rgba(255,255,255,0.05); padding:15px; border-radius:8px; text-align:center;">
                    <div style="font-size:0.8rem; color:#94a3b8;">VALOR TOTAL EM ESTOQUE</div>
                    <div style="font-size:1.5rem; font-weight:bold; color:#10b981;">R$ ${totalValorEstoque.toFixed(2)}</div>
                </div>
                <div style="background:rgba(255,255,255,0.05); padding:15px; border-radius:8px; text-align:center;">
                    <div style="font-size:0.8rem; color:#94a3b8;">GIRO DE ESTOQUE (30D)</div>
                    <div style="font-size:1.5rem; font-weight:bold; color:#3b82f6;">${resumo.reduce((acc,r) => acc + r.vendido, 0)} un</div>
                </div>
            </div>
        ...
        RelatoriosEnterprise.exibirModal("📈 Relatório Mensal de Estoque", "Performance dos últimos 30 dias", htmlTabela, null);
    },
    ...
    relatorioReposicao: async () => {
        ...
        const detalharVendedor = await NaxioUI.confirm(
            '👤 Detalhar por Vendedor?',
            'Deseja que os itens no relatório sejam separados por qual vendedor realizou a venda?'
        );
        ...
        if (orders) {
            orders.forEach(o => {
                ...
                itens.forEach(i => {
                    ...
                    const g = (detalharVendedor && i.garcom) ? ` <span style="color:#000; font-weight:bold; font-size:0.8em;"><br>👤 Vendedor: ${i.garcom}</span>` : '';
            ...
            todasComandasFechadas.forEach(c => {
                ...
                if (c.items && Array.isArray(c.items)) {
                    c.items.forEach(i => {
                        ...
                        const g = (detalharVendedor && i.garcom) ? ` <span style="color:#ef4444; font-size:0.75em;"><br>⚠️ Interno [Vendedor: ${i.garcom}]</span>` : '';
...
const PainelRelatorios = {
    open: async () => {
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 1200px; width: 98%; max-height:90vh; background: var(--surface); color: var(--text-color);">
                <div class="modal-header">
    ...
    openSalesHistory: async () => {
        ...
        <div id="sales-history-modal" class="modal-overlay" style="display:flex; z-index:10003;">
            <div class="modal-content" style="max-width:1200px; width: 98%; max-height:85vh;">
                <div class="modal-header">
