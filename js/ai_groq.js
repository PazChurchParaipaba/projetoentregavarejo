...
const GroqAI = {
    ...
    toggleChat: () => {
        ...
        if (modal.style.display === 'flex') {
            ...
            if (GroqAI.history.length === 0) {
                if (!GroqAI.apiKey) {
                    GroqAI.showApiKeyConfiguration();
                } else {
    ...
    showApiKeyConfiguration: () => {
        ...
        body.innerHTML = `
            <div style="display:flex; flex-direction:column; gap:15px; padding:10px; text-align:center;">
                <div style="font-size:2.5rem; color:#a855f7;"><i class="ri-key-2-line"></i></div>
                <h4 style="margin:0; color:white;">Configurar Chave Groq API</h4>
                <p style="font-size:0.85rem; color:var(--ai-text-muted); margin:0; line-height:1.4;">
                    Para conversar com a inteligência artificial, você precisa de uma chave API da Groq. É gratuita e pode ser obtida no painel da Groq.
                </p>
                <a href="https://console.groq.com/keys" target="_blank" style="font-size:0.8rem; color:#6366f1; text-decoration:underline; font-weight:bold;">
                    Obter Chave API Groq <i class="ri-external-link-line"></i>
                </a>
                <div style="display:flex; flex-direction:column; gap:5px; text-align:left; margin-top:10px;">
                    <label style="font-size:0.75rem; color:var(--ai-text-muted); font-weight:bold;">Sua API Key:</label>
                    <input type="password" id="groq-key-input" class="ai-input" placeholder="gsk_..." style="width:100%;" value="${GroqAI.apiKey || ''}">
                </div>
                <button class="btn btn-primary" onclick="GroqAI.saveApiKeyFromInput()" style="background:linear-gradient(135deg, #6366f1, #a855f7); border:none; height:40px; font-weight:bold; margin-top:5px; border-radius:10px; color:white; cursor:pointer;">
                    Salvar e Conectar
                </button>
            </div>
        `;
    ...
    saveApiKeyFromInput: () => {
        ...
        if (!key) {
            GroqAI.showToast("Digite uma chave válida", "error");
            return;
        }
        ...
        GroqAI.addMessage('bot', "Conexão neural ativada com sucesso! Olá! Sou o cérebro do Naxio Enterprise. Posso te ajudar a gerenciar vendas, estoque, clientes e tirar qualquer dúvida sobre o sistema. O que faremos agora?");
    },
                ...
                <div class="ai-window">
                    <div class="ai-header">
                        ...
                        <div style="display:flex; gap:10px; align-items:center;">
                            <div id="ai-speaking-indicator" style="display:none; margin-right:5px; animation:ai-pulse-mic 1.5s infinite;">
                                <img src="https://cdn-icons-png.flaticon.com/512/8649/8649607.png" style="width:24px; height:24px; filter: drop-shadow(0 0 5px #00ffff) brightness(1.3) cyan;">
                            </div>
                            <button class="btn btn-sm" onclick="GroqAI.showApiKeyConfiguration()" title="Configurar Chave API" style="background:transparent; border:none; color:white; cursor:pointer; padding:0; display:flex; align-items:center; justify-content:center;"><i class="ri-key-2-line" style="font-size:1.2rem;"></i></button>
                            <button class="btn btn-sm" onclick="GroqAI.clearHistory()" title="Limpar Histórico" style="background:transparent; border:none; color:white; cursor:pointer; padding:0; display:flex; align-items:center; justify-content:center;"><i class="ri-delete-bin-7-line" style="font-size:1.2rem;"></i></button>
                            <button class="btn btn-sm" onclick="GroqAI.toggleChat()" title="Fechar Chat" style="background:transparent; border:none; color:white; cursor:pointer; padding:0; display:flex; align-items:center; justify-content:center;"><i class="ri-close-line" style="font-size:1.2rem;"></i></button>
                        </div>
                    </div>
                    ...
                    <div class="ai-footer">
                        <button class="ai-btn-action ai-btn-mic" id="ai-mic-btn" onclick="GroqAI.startVoiceRecognition(false)"><i class="ri-mic-fill"></i></button>
    ...
    startVoiceRecognition: (isEmbedded = false) => {
        const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
        if (!SpeechRecognition) return GroqAI.showToast("Voz não suportada", "error");

        const recognition = new SpeechRecognition();
        recognition.lang = 'pt-BR';
        
        const btnId = isEmbedded ? 'btn-ai-mic-embedded' : 'ai-mic-btn';
        const inputId = isEmbedded ? 'ai-input-embedded' : 'ai-input-field';
        ...
        recognition.onresult = (event) => {
            ...
            if (input) {
                input.value = text;
                if (isEmbedded) {
                    GroqAI.sendMessageFromEmbedded();
                } else {
                    GroqAI.handleSend();
                }
            }
    ...
    sendMessageFromEmbedded: async () => {
        const input = document.getElementById('ai-input-embedded');
        if (!input) return;
        ...
        // Adiciona mensagem do usuário no chat embutido
        const embeddedBody = document.getElementById('ai-embedded-messages');
        if (embeddedBody) {
            const userDiv = document.createElement('div');
            userDiv.className = 'ai-bubble-user';
            userDiv.style = "align-self: flex-end; background: var(--ai-primary); color: white; padding: 8px 12px; border-radius: 12px 12px 0 12px; font-size: 0.85rem; max-width: 85%; margin-bottom: 8px; word-break: break-word;";
            userDiv.innerText = text;
            embeddedBody.appendChild(userDiv);
        ...
        if (!GroqAI.apiKey) {
            if (embeddedBody) {
                ...
                errorDiv.className = 'ai-bubble-bot';
                errorDiv.style = "align-self: flex-start; background: rgba(239, 68, 68, 0.1); color: #ef4444; padding: 8px 12px; border-radius: 12px 12px 12px 0; font-size: 0.85rem; max-width: 85%; margin-bottom: 8px; border: 1px solid rgba(239, 68, 68, 0.2);";
                errorDiv.innerHTML = `⚠️ Chave API do Groq não configurada. Clique no botão de IA flutuante (canto inferior direito) para configurar sua chave API do Groq.`;
                embeddedBody.appendChild(errorDiv);
                embeddedBody.scrollTop = embeddedBody.scrollHeight;
            }
            return;
        }
        ...
        let tempBotDiv = null;
        if (embeddedBody) {
            tempBotDiv = document.createElement('div');
            tempBotDiv.className = 'ai-bubble-bot';
            tempBotDiv.style = "align-self: flex-start; background: rgba(255,255,255,0.05); color: var(--ai-text-muted); padding: 8px 12px; border-radius: 12px 12px 12px 0; font-size: 0.85rem; max-width: 85%; margin-bottom: 8px;";
            tempBotDiv.innerHTML = `<div class="ai-typing"><div class="ai-dot"></div><div class="ai-dot"></div><div class="ai-dot"></div></div>`;
            embeddedBody.appendChild(tempBotDiv);
            embeddedBody.scrollTop = embeddedBody.scrollHeight;
        }
        ...
        try {
            ...
            GroqAI.history.push({ role: 'user', content: text });
            ...
            if (tempBotDiv) tempBotDiv.remove();
            ...
            if (data.choices && data.choices[0]) {
                ...
                // Adiciona ao histórico do assistente
                GroqAI.history.push({ role: 'assistant', content: cleanText });
                sessionStorage.setItem('ai_chat_history', JSON.stringify(GroqAI.history));

                if (embeddedBody) {
                    const botDiv = document.createElement('div');
                    botDiv.className = 'ai-bubble-bot';
                    botDiv.style = "align-self: flex-start; background: rgba(0,0,0,0.2); color: var(--ai-text); padding: 8px 12px; border-radius: 12px 12px 12px 0; font-size: 0.85rem; max-width: 85%; margin-bottom: 8px; border: 1px solid var(--ai-border);";
                    botDiv.innerHTML = cleanText.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>').replace(/`(.*?)`/g, '<code>$1</code>').replace(/\n/g, '<br>');
                    embeddedBody.appendChild(botDiv);
                    embeddedBody.scrollTop = embeddedBody.scrollHeight;
                }
            ...
            if (embeddedBody) {
                ...
                errorDiv.className = 'ai-bubble-bot';
                errorDiv.style = "align-self: flex-start; background: rgba(239, 68, 68, 0.1); color: #ef4444; padding: 8px 12px; border-radius: 12px 12px 12px 0; font-size: 0.85rem; max-width: 85%; margin-bottom: 8px; border: 1px solid rgba(239, 68, 68, 0.2);";
                errorDiv.innerText = "❌ Tive um problema em processar isso. Verifique sua chave de API ou conexão.";
                embeddedBody.appendChild(errorDiv);
                embeddedBody.scrollTop = embeddedBody.scrollHeight;
            }
            console.error("AI Embedded Error:", error);
        }
    },
    ...
    speak: (text) => {
        ...
        // Animação de fala discreta no cabeçalho
        const speakingIndicator = document.getElementById('ai-speaking-indicator');
