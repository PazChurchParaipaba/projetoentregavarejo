const CONFIG = {
    ...
    categoriesByStoreType: {
        'Restaurante': ['Lanches', 'Bebidas', 'Pratos', 'Porções', 'Sobremesas', 'Drinks', 'Entradas', 'Combos', 'Outros'],
        'Autopeças': ['Mecânica', 'Elétrica', 'Suspensão', 'Freios', 'Motor', 'Óleos & Filtros', 'Acessórios', 'Pneus', 'Outros'],
        'Roupas': ['Masculino', 'Feminino', 'Infantil', 'Moda Praia', 'Calçados', 'Acessórios', 'Outros'],
        'Varejo': ['Geral', 'Eletrônicos', 'Decoração', 'Brinquedos', 'Papelaria', 'Outros'],
        'Outros': ['Geral', 'Serviços', 'Diversos']
    },

    // Função para obter categorias do ramo da loja
    getCategoriesForStoreType: function (storeType = 'Outros') {
        const type = storeType || 'Outros';
        
        // Normalização agressiva para match
        const clean = type.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        
        if (clean.includes('restaurante') || clean.includes('comida') || clean.includes('lanche')) 
            return this.categoriesByStoreType['Restaurante'];
            
        if (clean.includes('auto') || clean.includes('peca') || clean.includes('oficina') || clean.includes('mecanic'))
            return this.categoriesByStoreType['Autopeças'];

        if (clean.includes('roupa') || clean.includes('vestuario') || clean.includes('moda'))
            return this.categoriesByStoreType['Roupas'];

        if (clean.includes('varejo') || clean.includes('mercado') || clean.includes('loja'))
            return this.categoriesByStoreType['Varejo'];

        return this.categoriesByStoreType[type] || this.categoriesByStoreType['Outros'];
    },

    subCategoriesAutopecas: ['Injeção Eletrônica', 'Câmbio', 'Direção', 'Ar Condicionado', 'Lataria', 'Iluminação', 'Som e Vídeo'],
    subCategoriesRoupas: ['Camisetas', 'Calças', 'Vestidos', 'Biquinis', 'Tênis', 'Sapatos', 'Bonés']
};
...
const App = {
    ...
    utils: {
        ...
        isRestaurante: () => {
            ...
            return tl.includes("restaurante") || tl.includes("comida") || tl.includes("lanche") || tl.includes("gastronomia") || tl.includes("pizza") || tl.includes("hamburguer") || tl.includes("bar");
        },
        setupPWA: () => {
        ...
        setupCategories: () => {
            ...
            if (catList) {
                const tipoLoja = App.state.currentStore?.tipo_loja || 'Outros';
                const categories = CONFIG.getCategoriesForStoreType(tipoLoja);
    ...
    router: {
        go: (viewId) => {
            ...
            if (!viewEl) {
                console.error("❌ ERRO DE ROTA: View não encontrada ->", viewId);
                // Fallback: Se for PDV e não achar, tenta injetar de emergência
                if (viewId === 'pos' && typeof Varejo !== 'undefined' && Varejo.injectHTML) {
                    console.log("🛠️ Injetando view-pos de emergência via Router...");
                    Varejo.injectHTML();
                    // Tenta pegar de novo após injeção
                    const reCheck = document.getElementById('view-pos');
                    if (reCheck) {
                        reCheck.classList.add('active');
                        return;
                    }
                }
                return;
            ...
            App.router.renderNav();
            if (viewId === 'home') {
                // Splash screen logic is handled in index.html load listener
            }
        },
        renderNav: () => {
