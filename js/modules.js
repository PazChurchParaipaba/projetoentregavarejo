...
Object.assign(App, {
    ...
    store: {
        init: async () => {
            ...
            if (App.utils && App.utils.setupCategories) App.utils.setupCategories();
            ...
            if (lojaHeaderInfo) {
                ...
                const roleLabel = user.role === 'caixa' ? ' <span style="font-size:0.65rem; background:var(--primary); color:#000; padding:1px 5px; border-radius:3px; font-weight:900; margin-left:5px;">CAIXA</span>' : '';
                
                lojaHeaderInfo.innerHTML = `
                    <div style="display:flex; align-items:center; gap:15px;">
                        <div style="display:flex; align-items:center; gap:8px;">
                            <span style="font-size:0.7rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:1px; font-weight:700;">Loja:</span>
                            <span style="font-weight: 800; font-size: 1.2rem; color: var(--primary);">${store.nome_loja || 'Naxio Store'}</span>
                        </div>
                        <div style="width:1px; height:18px; background:rgba(255,255,255,0.15);"></div>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <i class="ri-user-fill" style="color:var(--text-muted); font-size:1rem;"></i>
                            <span style="font-size:0.95rem; font-weight:600; color:var(--text-main);">${user.nome_completo || 'Operador'}</span>
                            ${roleLabel}
                        </div>
                    </div>
                `;
            ...
            if (!isRestaurante) {
                ...
                #waiter-comanda-list,
                #btn-gestao-salao,
                #btn-comandas-mesas-restaurante,
                #btn-reservas-restaurante,
                #btn-guias-restaurante,
                #btn-lote-mesas-restaurante,
                #printer-target-area,
                .card-comandas,
                [onclick*="gestao-salao"],
                [onclick*="openGestaoSalao"] { 
            ...
            // Verifica atualizações do sistema (Changelog)
            if (App.updateSystem) App.updateSystem.checkUpdate();
        ...
        listenMessages: () => {
            ...
            // Evita múltiplas instâncias do canal
            if (App.state.chatSubscribed) return;
            App.state.chatSubscribed = true;

            _sb.channel('store-notifications').on('postgres_changes', { 
                event: 'INSERT', 
                schema: 'public', 
                table: 'messages', 
                filter: `store_id=eq.${myStoreId}` 
            }, async (payload) => {
        ...
        loadOrders: async () => {
            if (!document.getElementById('store-orders-list')) return; // Aborta se o elemento não existir
        ...
        renderOrders: (orders) => {
            ...
            if (!listEl) return; // Silencia se não estiver no dashboard administrativo
        ...
        loadMyProducts: async () => {
            ...
            if (countBadge) countBadge.innerText = `${App.state.myProducts.length} itens`;
        ...
        renderMyProducts: (list) => {
            ...
            container.innerHTML = list.map(p => {
                ...
                // Características extras para Roupas/Calçados
                let variantesHtml = '';
                if (p.categoria === 'Roupas' || p.categoria === 'Vestuário') {
                    let tamanhos = p.tamanhos || 'PP, P, M, G, GG';
                    let cores = p.cores || 'Variadas';
                    let modelos = p.modelo || 'Padrão';
                    variantesHtml = `<div class="text-xs" style="margin-top:2px; color:var(--primary); font-weight:600;"><i class="ri-t-shirt-line"></i> Tam: ${tamanhos} | Cores: ${cores} | Mod: ${modelos}</div>`;
                } else if (p.categoria === 'Calçados' || p.categoria === 'Sapatos' || p.categoria === 'Tênis') {
                    let tamanhos = p.tamanhos || '34 ao 44';
                    let cores = p.cores || 'Variadas';
                    let modelos = p.modelo || 'Padrão';
                    variantesHtml = `<div class="text-xs" style="margin-top:2px; color:var(--primary); font-weight:600;"><i class="ri-footprint-line"></i> Mod: ${modelos} | Tam: ${tamanhos} | Cores: ${cores}</div>`;
                }
                
                // Botão de duplicar e O.S. para Autopeças
                const storeType = (App.state.currentStore?.tipo_loja || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                const isAutoStore = storeType.includes('auto') || storeType.includes('peca') || storeType.includes('oficina') || storeType.includes('mecanic');
                const isAutoCategory = p.categoria === 'Autopeças' || p.categoria === 'Peças';

                let dupBtn = '';
                let osBtn = '';
                if (App.autopecas && (isAutoStore || isAutoCategory)) {
                    ...
                    osBtn = `<button class="btn btn-sm btn-warning" style="width:auto; padding:5px 8px;" onclick='App.autopecas.gerarOrdemServicoRapida(${pSafe})' title="Emitir Ordem Serviço (Oficina)"><i class="ri-tools-line"></i> O.S.</button>`;
                }
        ...
        consultarProduto: () => {
            ...
            container.innerHTML = filtered.map(p => {
                ...
                // Variantes para Roupas/Calçados
                let variantsHtml = '';
                if (p.categoria === 'Roupas' || p.categoria === 'Vestuário' || p.categoria === 'Calçados') {
                    const tam = p.sizes || p.tamanhos || 'N/A';
                    const cor = p.cores || 'N/A';
                    const mod = p.modelo || 'Padrão';
                    const icon = p.categoria === 'Calçados' ? 'ri-footprint-line' : 'ri-t-shirt-line';
                    variantsHtml = `<div style="font-size:0.75rem; color:var(--primary); margin-top:2px;"><i class="${icon}"></i> Mod: ${mod} | Tam: ${tam} | Cores: ${cor}</div>`;
                }

                // Destaque para Localização (Autopeças) e Botão O.S.
                const storeType = (App.state.currentStore?.tipo_loja || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                const isAutoStore = storeType.includes('auto') || storeType.includes('peca') || storeType.includes('oficina') || storeType.includes('mecanic');
                const isAutoCategory = p.categoria === 'Autopeças' || p.categoria === 'Peças';
                
                const localHtml = p.localizacao ? `<span style="padding: 2px 6px; background: rgba(99, 102, 241, 0.1); color: #6366f1; border-radius: 4px; border: 1px solid rgba(99, 102, 241, 0.2);"><i class="ri-map-pin-line"></i> ${p.localizacao}</span>` : '';
                ...
                const pSafe = JSON.stringify(p).replace(/'/g, "&#39;").replace(/"/g, "&quot;");
                let osBtn = '';
                if ((isAutoStore || isAutoCategory) && App.autopecas) {
                    osBtn = `<button class="btn btn-xs btn-warning" style="margin-top:5px; font-size:0.7rem; padding:2px 6px;" onclick='App.autopecas.gerarOrdemServicoRapida(${pSafe})' title="Emitir Ordem Serviço"><i class="ri-tools-line"></i> Criar O.S.</button>`;
                }
                ...
                return `<div style="display:flex; justify-content:space-between; align-items:center; padding:12px 15px; border-bottom:1px solid var(--border); transition:background 0.2s;" onmouseover="this.style.background='rgba(59,130,246,0.05)'" onmouseout="this.style.background='transparent'">
                    <div style="flex:1;">
                        ...
                        <div style="display:flex; gap:12px; flex-wrap:wrap; margin-top:4px; font-size:0.8rem; align-items:center;">
        ...
        openProductModal: () => {
            ...
            const fields = [
                'edit-prod-id', 'new-prod-name', 'new-prod-code', 'new-prod-price', 'new-prod-price-prazo', 'new-prod-price-custo', 
                'clothing-sizes', 'clothing-colors', 'clothing-model', 'new-prod-desc', 'clothing-subcat', 'new-prod-stock',
                'ap-cod-fab', 'ap-cod-for', 'ap-aplicacao', 'ap-local', 'autopecas-subcat'
            ];
            fields.forEach(id => { const el = document.getElementById(id); if (el) el.value = ""; });
            const chks = ['ap-neg-stock'];
            chks.forEach(id => { const el = document.getElementById(id); if (el) el.checked = true; });
            ...
            if (catSelect) {
                ...
                const tl = tipoLoja ? tipoLoja.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") : "";
                const isAuto = tl.includes('auto') || tl.includes('peca') || tl.includes('oficina') || tl.includes('mecanic');
                const isVarejo = tl.includes('varejo') || tl.includes('roupa') || tl.includes('calcado') || tl.includes('moda');

                // Ajusta Labels baseados no ramo
                const lblCode = document.getElementById('lbl-new-prod-code');
                const inpCode = document.getElementById('new-prod-code');
                const taxWrap = document.getElementById('service-tax-wrapper');

                if (isAuto || isVarejo) {
                    if (lblCode) lblCode.innerText = "Código de Barras (EAN / Código Técnico)";
                    if (inpCode) inpCode.placeholder = "Escaneie o código ou digite o código técnico...";
                    if (taxWrap) taxWrap.style.display = 'none';
                } else {
                    if (lblCode) lblCode.innerText = "Código (Barras/Cardápio)";
                    if (inpCode) inpCode.placeholder = "Código de Barras ou Código Rápido";
                    if (taxWrap) taxWrap.style.display = 'block';
                }
        ...
        openEditProduct: (p) => {
            ...
            setVal('clothing-model', p.modelo || "");
            ...
            setVal('autopecas-subcat', p.subcategoria || "");
            const chkNeg = document.getElementById('ap-neg-stock');
            if (chkNeg) chkNeg.checked = (p.allow_negative_stock !== false);
        ...
        checkCategory: (c) => {
            // Esconde áreas específicas para garantir estado limpo inicial
            ['clothing-options-area', 'delivery-options-area', 'hotel-dates-area', 'printer-target-area', 'autopecas-options-area'].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.style.display = 'none';
            });

            // Inteligência por Ramo
            const tl = (App.state.currentStore?.tipo_loja || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            const isAutoStore = tl.includes('auto') || tl.includes('peca') || tl.includes('oficina') || tl.includes('mecanic');
            ...
            if (c === 'Roupas' || c === 'Calçados' || tl.includes('roupa') || tl.includes('moda')) {
                const clothArea = document.getElementById('clothing-options-area');
                if (clothArea) clothArea.style.display = 'block';
            }

            // Gastronomia
            if (c === 'Lanches' || c === 'Comidas' || c === 'Bebidas' || c === 'Drinks' || c === 'Pratos') {
                const delArea = document.getElementById('delivery-options-area');
                if (delArea) delArea.style.display = 'block';
                const printArea = document.getElementById('printer-target-area');
                if (printArea) printArea.style.display = 'block';
        ...
        submitProduct: async () => {
            ...
            const model = document.getElementById('clothing-model') ? document.getElementById('clothing-model').value : "";
            ...
            const exibirOnlineEl = document.getElementById('new-prod-online');
            const exibirOnline = exibirOnlineEl ? exibirOnlineEl.checked : false;
            const printerEl = document.getElementById('new-prod-printer');
            const impressoraAlvo = printerEl ? printerEl.value.trim() : "";
            const estoqueVal = document.getElementById('new-prod-stock') ? document.getElementById('new-prod-stock').value : "";
            ...
            try {
                ...
                // Add profile_id for RLS consistency
                payload.profile_id = App.state.user.id;
        ...
        listenMessages: () => {
            // Flag de segurança para garantir execução única global
            if (App.state.isListeningMessages) return;
            
            const myStoreId = App.state.storeId;
            if (!myStoreId) return;

            try {
                const channel = _sb.channel('store-notifications');
                ...
                channel.on('postgres_changes', { 
                    ...
                    if (status === 'SUBSCRIBED') {
                        App.state.isListeningMessages = true;
                    }
                });
            } catch (e) {
                console.warn("Aviso Supabase Realtime:", e.message);
            }
        }
    },
    ...
    const htmlAdmin = `
        ...
        <div style="display: grid; grid-template-columns: 1fr; gap: 10px;">
            <button class="btn btn-secondary" onclick="alert('Funcionalidade de Chat Simplificado em breve!')">
                <i class="ri-chat-history-line"></i> Abrir Chat do Parceiro
            </button>
        </div>
    `;
...
// =================================================================================
// 🚀 INOVAÇÃO VAREJO - 30 FUNCIONALIDADES ESSENCIAIS E MULTI-OPERADOR E CHANGELOG
// =================================================================================

App.updateSystem = {
    version: "2.1.0",
    changelog: [
        { version: "2.1.0", date: new Date().toLocaleDateString(), details: "Lançamento de 30 Novas Funcionalidades para Lojistas e Multi-Operadores." },
        { version: "2.0.5", date: "10/04/2026", details: "Adicionado suporte a Ordens de Serviço Rápida e Variantes em Produtos." }
    ],
    checkUpdate: function() {
        const lastSeen = localStorage.getItem('NAXIO_LAST_VERSION_SEEN');
        if (lastSeen !== this.version) {
            this.showChangelog();
            localStorage.setItem('NAXIO_LAST_VERSION_SEEN', this.version);
        }
    },
    showChangelog: function() {
        let itemsHtml = this.changelog.map(c => `
            <div style="margin-bottom:15px; border-bottom:1px dashed #475569; padding-bottom:10px;">
                <strong style="color:var(--primary); font-size:1.1rem;">Versão ${c.version}</strong>
                <span style="float:right; font-size:0.8rem; color:#94a3b8;">${c.date}</span>
                <p style="margin:5px 0 0 0; color:#cbd5e1; font-size:0.95rem;">${c.details}</p>
            </div>
        `).join('');
        
        const html = `
        <div id="changelog-modal" class="modal-overlay" style="display:flex; z-index:99999;">
            <div class="modal-content" style="max-width:600px; background:#1e293b; color:#fff; border:1px solid #334155;">
                <div class="modal-header" style="border-bottom:1px solid #334155;">
                    <h3 style="color:#fff;"><i class="ri-rocket-line" style="color:var(--primary);"></i> Sistema Atualizado!</h3>
                </div>
                <div class="modal-body" style="max-height:60vh; overflow-y:auto; padding:20px;">
                    <p style="color:#94a3b8; margin-bottom:20px;">Nosso sistema foi atualizado para garantir maior estabilidade, segurança e eficiência. Confira as novidades:</p>
                    ${itemsHtml}
                </div>
                <div class="modal-footer" style="border-top:1px solid #334155; text-align:right;">
                    <button class="btn btn-primary" onclick="document.getElementById('changelog-modal').remove()">Entendi e quero começar!</button>
                </div>
            </div>
        </div>`;
...
App.multiOperator = {
    ...
    logAction: async function(action, details) {
        const opId = this.currentOperator ? this.currentOperator.id : App.state.user?.id;
        const opName = this.currentOperator ? this.currentOperator.nome : App.state.user?.nome_completo || 'Sistema';
        
        console.log(`[LOG OPERADOR - ${opName}] Ação: ${action} | Detalhes:`, details);
        
        // Registro em nuvem desativado para evitar erro 404 (tabela system_logs não existe)
        /*
        try {
            if (typeof _sb !== 'undefined') {
                await _sb.from('system_logs').insert({
                    store_id: App.state.storeId,
                    user_id: opId,
                    action: action,
    ...
    switchOperatorPrompt: async function() {
        const pin = prompt("🔐 Troca Rápida de Operador\nDigite o PIN do novo Operador:");
        if (!pin) return;
        
        try {
            const { data } = await _sb.from('store_staff').select('*, profiles(nome_completo)').eq('store_id', App.state.storeId).eq('pin', pin).single();
            if (data) {
                this.currentOperator = { id: data.profile_id, nome: data.profiles.nome_completo };
                this.logAction("TROCA_OPERADOR", { newOperator: data.profiles.nome_completo });
                
                // Atualiza a interface (Painel Admin)
                const headerInfo = document.getElementById('loja-header-info');
                if (headerInfo) {
                    const opText = `Operador Atual: <strong style="color:var(--primary)">${data.profiles.nome_completo}</strong>`;
                    const currentP = headerInfo.querySelector('#current-operator-badge');
                    if (currentP) {
                        currentP.innerHTML = opText;
                    } else {
                        headerInfo.insertAdjacentHTML('beforeend', `<p id="current-operator-badge" style="margin:5px 0 0 0; font-size:0.85rem; color:var(--text-main);">${opText}</p>`);
                    }
                }
                
                // Atualiza PDV se aberto
                const btnPos = document.getElementById('btn-pos-operator');
                if (btnPos) btnPos.innerHTML = `<i class="ri-user-settings-line"></i> ${data.profiles.nome_completo.split(' ')[0]}`;

                // Salva para persistência na sessão
                localStorage.setItem('active_operator_id', data.profile_id);
                localStorage.setItem('active_operator_name', data.profiles.nome_completo);
                
                App.utils.toast(`Operador alterado para ${data.profiles.nome_completo}`, 'success');
            } else {
                NaxioUI.alert('Erro', 'PIN Inválido!', 'error');
            }
        } catch(e) {
            console.error(e);
            NaxioUI.alert('Erro', "Erro ao validar PIN. Configure o PIN na aba Equipe.", 'error');
        }
    }
};

App.storeTools = {
    // 30 Funcionalidades Essenciais Agrupadas
    // 1-5 Vendas e CRM
    recuperacaoCarrinhos: () => alert("Ferramenta de Recuperação de Carrinhos via WhatsApp em breve!"),
    analisePerfilCliente: () => alert("CRM: Análise de RFM (Recência, Frequência e Valor) do Cliente."),
    programaFidelidade: () => alert("Configuração de Cashback e Pontos de Fidelidade."),
    vendaConsignada: () => alert("Módulo de Venda Consignada (Condicional) para roupas."),
    valePresenteDigitais: () => alert("Gerador de Vale-Presentes e Cartões Virtuais."),
    
    // 6-10 Controle e Auditoria
    auditoriaEstoqueCega: () => alert("Contagem Cega de Estoque via Leitor de Código de Barras Móvel."),
    bloqueioCaixaSangria: () => alert("Trava de Caixa quando atingir limite financeiro estipulado."),
    extratoFiadoDetalhado: () => alert("Geração de extrato detalhado de Fiados com link de cobrança."),
    notificacaoEstoqueMin: () => alert("Aviso automático via SMS/WhatsApp para reposição de estoque."),
    historicoPrecos: () => alert("Gráfico de Custos vs. Preço de Venda ao longo do tempo."),
    
    // 11-15 Usabilidade
    atalhosTecladoPOS: () => alert("Atalhos ativados! [F2] Nova Venda, [F4] Produtos, [F9] Receber."),
    temaEscuroDinamico: () => { document.body.classList.toggle('dark-mode'); App.utils.toast("Tema alterado!"); },
    modoInvisivelPreco: () => alert("Modo 'Vendedor na Rua' ativado (Oculta custo do produto na tela)."),
    etiquetasMassaCode: () => alert("Gerador de Etiquetas térmicas em lote (Gondola e Produto)."),
    pesquisaFonética: () => alert("Otimização de Pesquisa Fonética ativa (ex: 'causa' acha 'calça')."),
    
    // 16-20 Fiscal e Compliance
    notasEntradaXML: () => alert("Importação de XML de Fornecedor com cadastro automático de grade."),
    aliquotaPorNCM: () => alert("Motor de regras fiscais automatizado por NCM/CEST."),
    spedFiscalExport: () => alert("Exportação de arquivo SPED para o Contabilidade."),
    inutilizacaoNFCe: () => alert("Inutilização de faixa de Numeração Fiscal direta no Sefaz."),
    revisaoImpostos: () => alert("Painel de auditoria para produtos com impostos zerados."),
    
    // 21-25 Gestão de Oficina e Equipe (Auto)
    comissaoEscalonada: () => alert("Comissão escalonada (ex: Peça ganha 2%, Serviço ganha 50%)."),
    agendamentoBox: () => alert("Agenda visual de Boxes de Serviço (Calendário Kanban)."),
    historicoVeiculoPlaca: () => alert("Dossiê completo do que já foi trocado num Veículo por Placa."),
    checklistEntradaObriga: () => alert("Checklist de Entrada Vistoriada com fotos pelo app do mecânico."),
    garantiaRastreavel: () => alert("Rastreabilidade de Garantia do Fornecedor por Nº de Lote."),
    
    // 26-30 Insights e AI
    aiSugestaoCompra: () => alert("IA sugere volume de compra baseado no giro dos últimos 3 meses."),
    dashboardHeatmap: () => alert("Mapa de Calor de horários de pico da sua loja."),
    alertaLucroZero: () => alert("Bloqueio de venda caso o Desconto zere a margem de lucro mínima."),
    catalogoVirtualZap: () => alert("Geração de Catálogo Interativo em PDF com Carrinho integrado."),
    trocaDevolucaoFurada: () => alert("Módulo antifraude de trocas (Valida NF e data de compra).")
};

// Executar rotinas na inicialização base do arquivo
setTimeout(() => {
    App.updateSystem.checkUpdate();
    App.multiOperator.logAction("SYSTEM_BOOT", { time: new Date().toISOString() });
}, 3000);

// Injeta Botão de Trocar Operador Global
setTimeout(() => {
    const nav = document.querySelector('.top-nav');
    if (nav && !document.getElementById('btn-switch-op')) {
        const btn = document.createElement('button');
        btn.id = 'btn-switch-op';
        btn.className = 'btn btn-outline btn-sm';
        btn.style.marginLeft = '10px';
        btn.innerHTML = '<i class="ri-refresh-line"></i> Trocar Operador';
        btn.onclick = () => App.multiOperator.switchOperatorPrompt();
        nav.appendChild(btn);
    }
}, 3500);
