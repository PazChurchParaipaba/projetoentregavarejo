if (typeof App !== 'undefined') {
    Object.assign(App, {
        autopecas: {
            init: async () => {
                console.log("🔧 Diagnóstico Autopeças: Iniciando verificação...");
                
                let type = App.state.currentStore?.tipo_loja || "";
                console.log("🏪 Tipo de loja detectado:", type);

                // Normaliza para comparação (remove acentos e lowercase)
                const cleanType = type.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                console.log("🧼 Tipo normalizado:", cleanType);

                // Aceita variações como: autopeças, auto peças, oficina, mecânica, auto center
                const isAuto = cleanType.includes('auto') || cleanType.includes('peca') || cleanType.includes('oficina') || cleanType.includes('mecanic') || cleanType.includes('pecas');

                if (!isAuto) {
                    console.log(`⚠️ Módulo Autopeças ignorado: O tipo "${type}" não foi identificado como automotivo.`);
                    return;
                }

                console.log("✅ Loja identificada como Automotiva. Injetando interface...");
            ...
            gerarOrdemServicoRapida: async (p) => {
                if(!confirm(`Deseja emitir uma Ordem de Serviço rápida para a peça: ${p.nome}?`)) return;
                
                // Prompt for basic info
                const name = await NaxioUI.prompt("Nome do Cliente", "Ex: Carlos Silva", "", "Balcão");
                if(!name && name !== "") return; // cancelled
                
                const vehicle = await NaxioUI.prompt("Veículo", "Ex: Gol G5", "", "Balcão Oficina");
                if(!vehicle && vehicle !== "") return;

                const { data: newOs, error } = await _sb.from('service_orders').insert({
                    store_id: App.state.storeId,
                    profile_id: App.state.user.id,
                    cliente_nome: name || 'Balcão',
                    veiculo_modelo: vehicle || 'Balcão Oficina',
                    status: 'aberta'
                }).select().single();
                
                if (error || !newOs) return alert("Erro ao criar OS.");
                ...
                // Recalcula total da OS e abre Painel
                App.autopecas.recalcOS(newOs.id);
                App.utils.toast("OS Gerada com sucesso!", "success");
            },
            ...
            saveOS: async () => {
                ...
                const { error } = await _sb.from('service_orders').insert({
                    store_id: App.state.storeId,
                    profile_id: App.state.user.id, // Correção RLS
                    cliente_nome: name,
                    ...
                    <div class="modal-content" style="max-width:800px; height:90vh;">
                        ...
                        <div class="modal-body">
                            <div class="tabs" style="display:flex; gap:10px; margin-bottom:20px;">
                                ...
                                <button class="btn btn-sm" style="background:transparent; border:none; opacity:0.1;" onclick="App.autopecas.calcProfitForTithe()">.</button>
                            </div>
            ...
            loadVales: async () => {
                ...
                const { data: vales } = await _sb.from('financial_records')
                    .select('*, profiles!funcionario_id(nome_completo)') // Join explícito pela FK funcionario_id
                    .eq('store_id', App.state.storeId)
            ...
            // 💎 FUNÇÃO SECRETA: CÁLCULO DE LUCRO LÍQUIDO E DÍZIMO
            calcProfitForTithe: async () => {
                App.utils.toast("Calculando balanço espiritual...", "info");
                
                const trintaDias = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
                
                // 1. Busca Receitas (Vendas + Contas Pagas)
                const { data: vendas } = await _sb.from('orders').select('total_pago').eq('store_id', App.state.storeId).gte('created_at', trintaDias).not('status', 'in', '("cancelado")');
                const { data: contasReceita } = await _sb.from('financial_records').select('valor').eq('store_id', App.state.storeId).eq('tipo', 'receita').eq('status', 'pago').gte('data_pagamento', trintaDias);
                
                // 2. Busca Despesas (Contas Pagas + Vales Pagos)
                const { data: contasDespesa } = await _sb.from('financial_records').select('valor').eq('store_id', App.state.storeId).eq('tipo', 'despesa').eq('status', 'pago').gte('data_pagamento', trintaDias);
                const { data: valesPagos } = await _sb.from('financial_records').select('valor').eq('store_id', App.state.storeId).eq('tipo', 'vale_funcionario').eq('status', 'pago').gte('data_pagamento', trintaDias);
                
                const totalBruto = (vendas?.reduce((a,v) => a + (v.total_pago || 0), 0) || 0) + (contasReceita?.reduce((a,c) => a + c.valor, 0) || 0);
                const totalDespesas = (contasDespesa?.reduce((a,c) => a + Math.abs(c.valor), 0) || 0) + (valesPagos?.reduce((a,v) => a + Math.abs(v.valor), 0) || 0);
                
                const lucroLiquido = totalBruto - totalDespesas;
                const dizimo = lucroLiquido * 0.1;

                const modalHtml = `
                    <div id="tithe-modal" class="modal-overlay" style="display:flex; z-index:10020; background:rgba(0,0,0,0.9);">
                        <div class="modal-content" style="max-width:400px; text-align:center; padding:30px; border:2px solid #fbbf24; background:#1e1b4b; color:white;">
                            <i class="ri-heart-pulse-fill" style="font-size:3rem; color:#fbbf24;"></i>
                            <h2 style="margin:15px 0;">Balanço Mensal</h2>
                            <p style="opacity:0.8; font-size:0.9rem;">Cálculo baseado nos últimos 30 dias de registros pagos.</p>
                            
                            <div style="margin:20px 0; display:flex; flex-direction:column; gap:10px;">
                                <div style="display:flex; justify-content:space-between;"><span>Faturamento Bruto:</span><span style="color:#4ade80;">R$ ${totalBruto.toFixed(2)}</span></div>
                                <div style="display:flex; justify-content:space-between;"><span>Despesas Totais:</span><span style="color:#f87171;">- R$ ${totalDespesas.toFixed(2)}</span></div>
                                <hr style="border:0; border-top:1px solid rgba(255,255,255,0.1);">
                                <div style="display:flex; justify-content:space-between; font-weight:bold; font-size:1.1rem;"><span>Lucro Líquido:</span><span>R$ ${lucroLiquido.toFixed(2)}</span></div>
                                <div style="display:flex; justify-content:space-between; color:#fbbf24; font-weight:800; font-size:1.2rem; margin-top:10px; padding:10px; border:1px dashed #fbbf24; border-radius:8px;">
                                    <span>🙏 Sugestão Dízimo:</span><span>R$ ${dizimo.toFixed(2)}</span>
                                </div>
                            </div>
                            
                            <button class="btn btn-primary btn-full" onclick="document.getElementById('tithe-modal').remove()">Fechar</button>
                        </div>
                    </div>
                `;
                document.body.insertAdjacentHTML('beforeend', modalHtml);
